...
| **create-skill** | Creación de skill: rama `feat/create-skill-<skill-id>`. Entregable: cápsula en paths.skillCapsules; definición en paths.skillsDefinitionPath; implementación Rust (.exe en `bin/`). | `paths.featurePath/create-skill-<skill-id>` (doc); cápsula vía `paths.skillCapsules[<skill-id>]` | [`SddIA/process/create-skill/`](./SddIA/process/create-skill/) |
...
| **[CREATE-SKILL]** | Skill, Crear skill, Nueva skill, Implementación skill Rust. | Cargar [`SddIA/process/create-skill/`](./SddIA/process/create-skill/). Orquestar ciclo de creación de skill (definición paths.skillsDefinitionPath, cápsula paths.skillCapsules, índice, Cúmulo; contrato skills-contract). |
