        ...
        try
        {
            ...
            try
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    // Paso 8: Catálogo geo en Admin (SSOT); Product no inserta tablas locales de geo
                    result.Steps.Add("8. Verificando política geo (SSOT Admin)...");
                ...
                result.Steps.Add("   ✓ Política geo (sin tablas locales) aplicada");
            }
            catch (Exception ex)
            {
